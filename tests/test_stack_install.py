...
class InstallContracts(unittest.TestCase):
    ...
    def test_partial_apply_rolls_back_managed_bytes(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp).resolve(); source = self.fixture_source(root); target = root / "target"; target.mkdir()
            (target / "tools").mkdir(); (target / "tools" / "tool.py").write_text("old\n")
            plan = installer.plan_runtime_install(source, target)
            with self.assertRaises(installer.InstallError):
                installer.apply_runtime_plan(plan, inventory_root=root / "private", fail_after=1)
            self.assertEqual((target / "tools" / "tool.py").read_text(), "old\n")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for failure in range(1, count + 1):
                with self.subTest(failure=failure):
                    target = root / "target"
                    target.mkdir()
                    (target / "VERSION").write_text("old-version")
                    (target / "VERSION").chmod(0o640)
                    with self.assertRaises(installer.InstallError):
                        installer.apply_runtime_plan(plan, inventory_root=root / "private", fail_after=failure)
                    self.assertEqual(list(target.iterdir()), [target / "VERSION"])
                    self.assertEqual((target / "VERSION").read_text(), "old-version")
                    self.assertEqual((target / "VERSION").stat().st_mode & 0o777, 0o640)
                    self.assertFalse((root / "private").exists())
                    (target / "VERSION").unlink(); target.rmdir()
