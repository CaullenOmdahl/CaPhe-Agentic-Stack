...
You are Claude, one coding agent using this repository's shared agentic stack.

Read `adapters/claude/CLAUDE.md`, then read `docs/canon.md`.
...
Keep `.agent/OWNERS.md` human gates, defect-detection evidence, independent PR review, and real-artifact
verification. Uncertain changes are behavioral. Pre-commit `FAST GREEN` is feedback; completion requires
`~/strict-mode/bin/strict-green-gate.sh --mode completion` and PR evidence.
