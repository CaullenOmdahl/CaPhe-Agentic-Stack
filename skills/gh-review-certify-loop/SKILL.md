...
description: Obtain one remote PR implementation review, fall back locally when unavailable, and address findings with bounded retries and persistent cooldowns.
---
...
UTC cooldowns, request reservations, review limits and concise reporting.
...
3. Prefer remote review. After explicit unavailability/quota failure or one bounded
   response window, record the evidence/time and use one independent local reviewer
...
5. Batch related fixes and test behavior. Review the changed delta and affected
   context after fixes; keep previous evidence for unchanged code. At most two
   fix-and-rereview rounds after initial review across both routes, then human
   arbitration. Do not reset the count when the SHA or reviewer changes.
